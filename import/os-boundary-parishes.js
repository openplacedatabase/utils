...
jsonPipe.pipe(parser).on('data', function(data) {
  ...
  currentPlace.names = [
    feature.properties.NAME+', '+toTitleCase(feature.properties.FILE_NAME.split('_')[0])+', England',
    feature.properties.NAME+', '+toTitleCase(feature.properties.FILE_NAME.split('_').join(' '))+', England',
  ];
